...
class GPage extends SecureGamePage
{
    ...
    public function GPage()
    {
        ...
        while ($i < $c) {
            if (0 < $this->plusTable[$i]["time"]) {
                $this->plusTable[$i]["time"] = ceil(
                    $this->plusTable[$i]["time"] /
                        $this->gameMetadata["game_speed"]
                );
            }
            ++$i;
        }
    ...
    public function load()
    {
        ...
        if ($this->selectedTabIndex == 3) {
            ...
            while ($this->dataList2->next()) {
                ...
                if (
                    $this->dataList2->row["villages_count"] >= 2 &&
                    $this->dataList2->row["invite_by"] == $Id &&
                    $this->dataList2->row["show_ref"] == 0 &&
                    $this->dataList2->row["last_ip"] != $last_ip
                ) {
        ...
        if ($this->selectedTabIndex == 0) {
            $this->packageIndex =
                ...
                0 < intval($_GET["id"]) &&
                intval($_GET["id"]) <=
                    sizeof($this->appConfig["plus"]["packages"])
                    ? intval($_GET["id"]) - 1
                    : 0 - 1;
        } elseif (
        ...
        if (isset($_GET["backtroops0"])) {
            ...
            if ($gold_num >= $gback) {
                $result = $this->queueModel->provider->fetchResultSet(
                    "SELECT * FROM `p_queue` WHERE to_village_id = '" .
                        $vid .
                        "' AND proc_type='12'"
                );
                $this->queueModel->provider->executeQuery2(
                    "UPDATE p_players SET gold_num =gold_num - " .
                        $gback .
                        " WHERE id='" .
                        $this->player->playerId .
                        "'"
                ...
                while ($result->next()) {
                    ...
                    if ($end - $time <= 30) {
                        ...
                        $in_end = date(
                            "Y-m-d G:i:s",
                            strtotime("-144000 seconds")
                        );
                        $this->queueModel->provider->executeQuery(
                            "UPDATE p_queue set end_date='" .
                                $in_end .
                                "' WHERE id = '" .
                                $result->row["id"] .
                                "'"
                ...
                $tatarzx->provider->executeQuery(
                    "INSERT INTO `p_plus` (`pid`, `date`, `gold`, `where`) VALUES ('" .
                        $n .
                        "', '" .
                        $d .
                        "', '" .
                        $gq .
                        "', 'انهاء التعزيزات فوراً');"
                );
                //end pgold
                $this->redirect("village1.php");
            } else {
    ...
    public function getRemainingPlusTime($action)
    {
        ...
        if (isset($tasks[constant("QS_PLUS" . ($action + 1))])) {
            $time =
                $tasks[constant("QS_PLUS" . ($action + 1))][0][
                    "remainingSeconds"
                ];
        }
        return 0 < $time
            ? time_remain_lang .
                    " <span id=\"timer1\">" .
                    WebHelper::secondstostring($time) .
                    "</span> " .
                    time_hour_lang
            : "";
    }
    ...
    public function getPlusAction($action)
    {
        ...
        if ($action == 5 || $action == 7) {
            return "<a href=\"plus.php?t=2&a=" .
                $action .
                "&k=" .
                $this->data["update_key"] .
                "\">" .
                plus_text_activatefeature .
                "</a>";
        }
        if ($action == 6) {
            return $this->hasMarketplace()
                ? "<a href=\"build.php?bid=17&t=3\">" .
                        plus_text_gotomarket .
                        "</a>"
                : "<span class=\"none\">" . plus_text_gotomarket . "</span>";
        }
        $tasks = $this->queueModel->tasksInQueue;
        return isset($tasks[constant("QS_PLUS" . ($action + 1))])
            ? "<a href=\"plus.php?t=2&a=" .
                    $action .
                    "&k=" .
                    $this->data["update_key"] .
                    "\">" .
                    plus_text_extendfeature .
                    "</a>"
            : "<a href=\"plus.php?t=2&a=" .
                    $action .
                    "&k=" .
