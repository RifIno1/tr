...
if (((($troopId != 99 AND $_c <= 8) AND $tlevel < 20) AND $researches_done == 1))
{
$this->troopsUpgrade[$troopId] = $tlevel;
continue;
}
}
if (((((((isset ($_GET['a']) AND isset ($_GET['k'])) AND $_GET['k'] == $this->data['update_key']) AND !isset ($this->queueModel->tasksInQueue[$this->troopsUpgradeType])) AND isset ($this->troopsUpgrade[intval ($_GET['a'])])) AND !$this->isGameTransientStopped ()) AND !$this->isGameOver ()))
{
...
{
$this->selectedTabIndex = ((((isset ($_GET['t']) AND is_numeric ($_GET['t'])) AND 1 <= intval ($_GET['t'])) AND intval ($_GET['t']) <= 3) ? intval ($_GET['t']) : 0);
...
$firstplayerip = "1";
$secoundplayerip = "2";
if($firstplayerip == $secoundplayerip) {
$this->merchantProperty['showError4'] = TRUE;
}
} else {
$firstplayerip = "1";
$secoundplayerip = "2";
...
if ( !$this->merchantProperty['showError'] && $doSend && !$this->isGameTransientStopped( ) && !$this->isGameOver( ) && $firstplayerip != $secoundplayerip )
{
$this->merchantProperty['confirm_snd'] = FALSE;
...
{
$resources1 = array ('1' => ((isset ($_POST['rid1']) AND intval ($_POST['rid1']) == 1) ? intval ($_POST['m1']) : 0), '2' => ((isset ($_POST['rid1']) AND intval ($_POST['rid1']) == 2) ? intval ($_POST['m1']) : 0), '3' => ((isset ($_POST['rid1']) AND intval ($_POST['rid1']) == 3) ? intval ($_POST['m1']) : 0), '4' => ((isset ($_POST['rid1']) AND intval ($_POST['rid1']) == 4) ? intval ($_POST['m1']) : 0));
$resources2 = array ('1' => ((isset ($_POST['rid2']) AND intval ($_POST['rid2']) == 1) ? intval ($_POST['m2']) : 0), '2' => ((isset ($_POST['rid2']) AND intval ($_POST['rid2']) == 2) ? intval ($_POST['m2']) : 0), '3' => ((isset ($_POST['rid2']) AND intval ($_POST['rid2']) == 3) ? intval ($_POST['m2']) : 0), '4' => ((isset ($_POST['rid2']) AND intval ($_POST['rid2']) == 4) ? intval ($_POST['m2']) : 0));
...
{
if ((((($this->isPost () AND isset ($_POST['m2'])) AND is_array ($_POST['m2'])) AND sizeof ($_POST['m2']) == 4) AND $this->gameMetadata['plusTable'][6]['cost'] <= $this->data['gold_num']))
{
...
$calcConsume = intval ($troopMetadata['training_time_consume'] / $this->gameSpeed * ($artefact / ($this->buildProperties['building']['level'] + 9)) * $timeFactor);
$newTask = new QueueTask ($this->troopsUpgradeType, $this->player->playerId, $calcConsume);
$newTask->threads = $num;
$newTask->villageId = $this->data['selected_village_id'];
...
$cpRate = $tempdata[1];
//$this->totalCpValue = ($result->row['elapsedTimeInSeconds']*($cpRate / 86400));
//$this->totalCpValue += $result->row['elapsedTimeInSeconds']*($cpRate/86400);
$this->neededCpValue = intval (($this->gameMetadata['cp_for_new_village'] / $this->gameSpeed)*$this->data['villages_count']);
//$this->cpValue = list (, $cpRate) = explode (' ', $result->row['cp'])[0];
//+= 'cpValue';
...
//= $this->cpValue;
//+= 'neededCpValue';
//= intval ($this->gameMetadata['cp_for_new_village'] / $this->gameSpeed);
}
$this->vcplist = 0;
$this->totalCpValue = 0;
...
{
$this->selectedTabIndex = (((isset ($_GET['t']) AND is_numeric ($_GET['t'])) AND intval ($_GET['t']) == 1) ? intval ($_GET['t']) : 0);
if ($this->selectedTabIndex == 0)
{
...
*/
$calcConsume = intval ($troopMetadata['training_time_consume'] / $this->gameSpeed * ($artefact / ($this->buildProperties['building']['level'] + 9))) * 12;
$newTask = new QueueTask ($this->troopsUpgradeType, $this->player->playerId, $calcConsume);
...
if($needUpgradeType == 1) $needUpgradeType = 0;
if (0 < $needUpgradeType)
{
switch ($needUpgradeType)
{
