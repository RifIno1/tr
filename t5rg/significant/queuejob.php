...
class QueueJobModel extends ModelBase
    {
        ...
        {
                $result = $this->provider->fetchResultSet ("SELECT id, name FROM p_players WHERE UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(registration_date)>259200 AND is_active = 0");
                while ($result->next())
...
$this->provider->executeQuery ('UPDATE p_villages SET people_count= 0 WHERE people_count < 0');
    ...
    public function setWeeklyMedals($week)
        ...
        foreach ($keyArray as $columnName => $index)
            ...
            if ($result != NULL)
                ...
                while ($result->next())
                    {
                    $points = $result->row[''.$columnName.''];
                    $medal = $index . ":" . ++$i . ":" . $week . ":" . $points;
    ...
    public function createTatarVillages()
        ...
        $m        = new RegisterModel();
        $result   = $m->createNewPlayer(tatar_tribe_player, "", "", 5, 0, tatar_tribe_villages, $map_size, PLAYERTYPE_TATAR, 9);
        if ($result['hasErrors'])
            {
            return;
            }
...
لقد بدأت الحرب فقد اعلنت أشارة الحرب بين (التتار) و اللاعبين
انطلقت 9 قرى في مواقع معينة لديها معجزة العالم عليك عزيزي اللاعب ان تحاول  الحصول علي على احد من قراها حتى لايسبقك الجميع وهناك جيوش دفاعية متنوعة قوية (التتار) عليك تدميرها للحصول على معجزة العالم والفوز باللعبة وتدافع عن قريتك ويساندك تحالفك حتى تصل الى المستوى 100 حتى تغلب الجميع وتحصل على جائزة من الذهب لسيرفر القادم.

<b>استراتجية قد تهمك في استحلال قرى من التتار</b>
بناء الجيوش بالسطبل والثكنة والحصول على اكبر جيش ثم تقسم الهجوم الى هجومين هجوم استكشاف ثم هجوم تدميري ويجب ان يكون لديك جيوش طاحنة لسفح دماء التتار والقضاء عليهم وتدريب زعماء لستحلال على القرية.

<b>بخصوص الذهب</b>
هل تريد نقل الذهب الى سيرفر اخر من سيرفراتنا عليك اتابع الخطوات التالية وهي التعامل مع الادمن وطلب منه نقل الذهب الى سيرفر اخر من احد السيرفرات عند انتهاء هاذا السيرفر او عند اعادة السيرفر نفسه.
...
$num = $tid == 49 || $tid == 50 ? 0 : mt_rand(50000, 1111111);
