...
public function getWine( )
    {
        return $this->provider->fetchScalar( "SELECT win_village_id,wintime,windata FROM g_settings" );
    }
    ...
    {
        $playerId = intval( $this->provider->fetchScalar( "SELECT  `id`,`total_people_count`,`name` FROM  `p_players` ORDER BY  `total_people_count` DESC LIMIT 1" ) );
        return $this->getPlayerDataById( $playerId );
    }
