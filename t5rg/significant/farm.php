...
class FarmList extends ModelBase{
	...
	function getFarmList( $playerId, $villageId ){
        return $this->provider->fetchResultSet( 'SELECT f.id, f.player_id, f.from_village_id, f.to_village_id, f.troops FROM p_farms f WHERE f.player_id=%s AND f.from_village_id=%s', array( $playerId, $villageId ) );
    }
