        ...
        if ( $this->isPost( ) )
        {
            ...
            else
            {
                ...
                foreach ( $_POST['t2'] as $tribeId => $troopArray )
                {
                    ...
                    $totalDefensePower = $defenseTroops[$tribeId];
                }
                $this->warResult = $m->getWarResult( $attackTroops, $defenseTroops, $totalDefensePower,$wallLevel , $isPlunderAttack, isset( $_POST['ktyp'] ) && intval( $_POST['ktyp'] ) == 2 ,$heroLevel);
                $m->dispose( );
            }
        }
    }
